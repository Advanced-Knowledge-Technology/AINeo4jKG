...
{passage}"""
    ...
    def full_question_answer(self, query): 
        try:
            # Lấy đối tượng và quan hệ từ câu hỏi
            objs, relations = self.query_to_objs_relations(query)
            
            if len(objs) == 0:
                return "Rất tiếc, chúng tôi không tìm thấy thông tin đối tượng mà bạn đề cập trong câu hỏi."

            full_text = self.objs_relations_to_fulltext(objs, relations)
            
            # Bắt lỗi khi gọi answer_query
            try:
                answer = self.answer_query(query, full_text)
                
                # Nếu answer không hợp lệ hoặc bị cấm bởi bộ lọc an toàn
                if not answer or "SAFETY" in str(answer):
                    return "Rất tiếc, không thể tạo câu trả lời do hạn chế về nội dung an toàn."
            ...
            # Bắt lỗi tổng quát nếu có bất kỳ lỗi nào xảy ra trong toàn bộ hàm
